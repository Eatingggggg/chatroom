...
try:
    # 從 Streamlit Secrets 讀取 Service Account JSON
    creds = Credentials.from_service_account_info(
        st.secrets["gcp_service_account"],
        scopes=SCOPES
    )
    ...
    # 打開 Google Sheet（用你的 Sheet 名稱）
    SHEET_NAME = "chatroom"  # 改成你的 Sheet 名稱
    sheet = client.open(SHEET_NAME).sheet1
    ...
    st.write("前 5 筆資料：")
    st.write(records[:5] if records else "Sheet 目前沒有資料")
